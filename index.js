...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('NekellHaxor Base', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'NekellHaxor ', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6289695073357@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, ``, img,but = [{buttonId: `${prefix}donasi`, buttonText: {displayText: `MAKA NYA JANGAN TENGIL.`}, type: 1}], options = {contextInfo: {mentionedJid: [num]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6289695073357@s.whatsapp.net","6289695073357@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289695073357@s.whatsapp.net"
